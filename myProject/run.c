...
my fyles: (.h)
cod.h heder for structurs and code functions
errors.h heder for errors functions
linked_list.h heder to declare  the struct list and his union struct variables and function related to lists

bit field:
command structur built of cits field
...
fileName=(char*)malloc(MAX_LINE_LENGTH*sizeof(char));
	...
	for(i=1;i<argc;i++)
	{
		...
		strtok(fileName,".");
		turn_to_am(f,fileName,line,first_word_in_line);/*turn the file to be after macro spreading*/
		fclose(f);
		...
		fclose(f);
		strtok(fileName,".");
		sprintf(line,"\t%d\t%d",ic,dc);/*turn the ic and dc into astring with tub-to print in the head of the ".ob" file*/
		insertPrint(print_list,line,ic_dc);
		printings(fileName);/* all printing function-if there is error the function will print all errors else the function will print to externy and entry files the externy and entry lables and will decode the file*/
		freePrintList(print_list);/*free the memory that eas allocate for print_list*/ 
	}
	return 0;
