...
void errorDoubleMacroName(char *macro_name,list *macro_list)
{
	...
	while(m!=NULL)
	{
		if(strcmp((*m).macro_name,macro_name)==0 && (*m).index==1)/*This macro name has already been defined*/
		{
...
void errorDoubleLabelName(char *lable_name,list *list_to_check)
{
char *error_line=(char*)malloc((MAX_ERROR_LINE+strlen(lable_name))*sizeof(char));
	
	if(get_leble_adress(list_to_check,lable_name)!=NULL)
	{
		insertPrint(print_list,"Error defined lable more than one time",error);
		sprintf(error_line,"lable name:%s	line num:%d",lable_name,lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
	}
free(error_line);
...
void checMacroName(char *str)
{
	...
	|| !strcmp("cmp", s) || !strcmp("rts", s) || !strcmp("jmp", s) || !strcmp("jsr", s) || !strcmp("bne", s))
	{
			...
			sprintf(error_line,"macro name:%s	line num%d",s,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
			return ;	
...
int checkCommas(char s[])
{
	...
	if ((str[start] == COMMA))
	{
		insertPrint(print_list,"Error - illegal comma before an operand",error);
		sprintf(error_line,"line num:%d",lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
	}
	if ((str[end] == COMMA))
	{
		insertPrint(print_list,"Error - illegal comma after an operand",error);
		sprintf(error_line,"line num:%d",lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
	}
	...
	if(j3 == 0 && j4 != 0)
	{
		insertPrint(print_list,"Error - illegal extra operand",error);
		sprintf(error_line,"line num:%d",lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
		free(str);
...
int checkNumber(char str[])
{
	...
	if (s[0] == IMMIDIAT) 
        	...
        	if (length<2)
		{
			...
			sprintf(error_line,"operand:%s	line num:%d",s,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
		free(s);
        	return 1;
			...
			sprintf(error_line,"operand:%d	line num:%d",val,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
			return 1;	
...
int checkLabel(char str[])
{
	...
	if(length>30)
	{
		...
		sprintf(error_line,"lable name:%s	line num%d",s,lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
		return 1;			
	}
	...
	if(('A' <= s[0] && s[0] <= 'Z') || ('a'<= s[0] && s[0]<='z'))
	{
		while(j<length)
		{
			...
			{
				insertPrint(print_list,"Error - illegal label name",error);
				sprintf(error_line,"lable name:%s	line num:%d",s,lineNum);
				insertPrint(print_list,error_line,error);
				insertPrint(print_list,"----------------------------------------------",error);
				free(s);
...
while ((end >= 0) && (isspace(s[end])))
	...
	if ((s[start] != '"') || (s[end] != '"'))
	{
		...
		sprintf(error_line,"the string:%s	line num:%d",s,lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
		free(s);
		return 1;
...
if ((str[start] == COMMA))
{
	insertPrint(print_list,"Error - illegal comma before the first number",error);
	sprintf(error_line,"line num:%d",lineNum);
	insertPrint(print_list,error_line,error);
	insertPrint(print_list,"----------------------------------------------",error);
}	
if ((str[end] == COMMA))
{
	insertPrint(print_list,"Error - illegal comma after last number",error);
	sprintf(error_line,"line num:%d",lineNum);
	insertPrint(print_list,error_line,error);
	insertPrint(print_list,"----------------------------------------------",error);
}  
...
while (i <= end)
{
	...
	while (i <= end && isspace(str[i]))
        ...
        if (first == COMMA && second == COMMA)
	{
		insertPrint(print_list,"Error - illegal commas",error);/* 2 commas */
		sprintf(error_line,"line num:%d",lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
		return 1;
	...
	if ((first != COMMA) && (second != COMMA) && (second != END) && (j2-j1 != 1))
	{
            	insertPrint(print_list,"Error - illegal commas",error);
		sprintf(error_line,"line num:%d",lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
		return 1;
...
while(number != NULL)
{
	if(number[0]==POSITIVE || number[0]==NEGETIVE || isdigit(number[0]))
	{
		...
		while(i<length)
		{
			if(!isdigit(number[i]))
			{
			insertPrint(print_list,"Error - illegal number",error);
			sprintf(error_line,"number:%c	line num:%d",number[i],lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			return 1;
			}
		...
		{
			insertPrint(print_list,"Error - number out of range",error);
			sprintf(error_line,"number:%d	line num%d",val,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);	
		}
	...
	{
		insertPrint(print_list,"Error - illegal number",error);
		sprintf(error_line,"number:%c	line num%d",number[0],lineNum);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);	
		return 1;
	}
...
int checkTeachingSentences(char str[])
{
	...
	if(!strcmp("mov", com) || !strcmp("add", com) || !strcmp("sub", com))
	{
		...
		if(operand1==NULL || operand2==NULL)
		{
			...
			sprintf(error_line,"operand1:%s  operand2:%s	line num:%d",operand1, operand2,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
			return 1;
		...
		if(checkRegister(operand1)==0 || checkNumber(operand1)==0 || checkLabel(operand1)==0)
		{
			...
			{
			insertPrint(print_list,"Error - The destination operand is illegal",error);
			sprintf(error_line,"line num:%d",lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
		...
		{
			insertPrint(print_list,"Error - the source operand is illegal",error);
			sprintf(error_line,"line num:%d",lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
	...
	else if(!strcmp("not", com) || !strcmp("clr", com) || !strcmp("inc", com) || !strcmp("dec", com) || !strcmp("red", com))
	{
		if(s[3] !=SPACE && s[3] != TAB) 
		{
		...
		{
			insertPrint(print_list,"Error - missing operand",error);
			sprintf(error_line,"the operand:%s	line num:%d",operand1,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
			return 1;
	...
	else if(!strcmp("jmp", com) || !strcmp("bne", com) || !strcmp("jsr", com))
	{
		...
		{
			insertPrint(print_list,"Error - missing operand",error);
			sprintf(error_line,"the operan:%s 	line num:%d",operand1,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
			return 1;
		...
		if(checkLabel(operand1)==0)
		{
			...
			{
			insertPrint(print_list,"Error - missing operand",error);
			sprintf(error_line,"the operan:%s 	line num:%d",operand2,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
			return 1;
			...
			if(checkRegister(operand2)==0 || checkNumber(operand2)==0 || checkLabel(operand2)==0)
			{
				...
				if(operand3==NULL)
				{
					...
					sprintf(error_line,"parameter:%s	line num:%d",operand3,lineNum);
					insertPrint(print_list,error_line,error);
					insertPrint(print_list,"----------------------------------------------",error);
					free(s);
					return 1;
				...
				{
					insertPrint(print_list,"Error - illegal parameter",error);
					sprintf(error_line,"parameter:%s	line num:%d",operand3,lineNum);
					insertPrint(print_list,error_line,error);
					insertPrint(print_list,"----------------------------------------------",error);
					free(s);
			...
			else
			{
				...
				{
					insertPrint(print_list,"Error - illegal parameter",error);
					sprintf(error_line,"parameter:%s	line num:%d",operand2,lineNum);
					insertPrint(print_list,error_line,error);
					insertPrint(print_list,"----------------------------------------------",error);
				...
				{
					insertPrint(print_list,"Error - illegal parameters",error);
					sprintf(error_line,"parameters:%s   %s	 line num:%d",operand2,operand3,lineNum);
					insertPrint(print_list,error_line,error);
					insertPrint(print_list,"----------------------------------------------",error);
					free(s);
					return 1;
		...
		{
			insertPrint(print_list,"Error - illegal operand",error);
			sprintf(error_line,"parameters:%s	line num:%d",operand1,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
			return 1;
	...
	else if(!strcmp("lea", com))
	{
		...
		{
			insertPrint(print_list,"Error - missing operand",error);
			sprintf(error_line,"the operands:%s %s	line num:%d",operand1,operand2,lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
			free(s);
			return 1;
	...
	else if(!strcmp("rts", com) || !strcmp("stop", com1))
	{
		...
		if ((s[end] == COMMA))
		{
			insertPrint(print_list,"Error - illegal comma",error);
			sprintf(error_line,"line num:%d",lineNum);
			insertPrint(print_list,error_line,error);
			insertPrint(print_list,"----------------------------------------------",error);
		}
...
void checkEntryNotDefined(list *entry_lable_list)
{
	...
	if(flag==OFF)
	{
		...
		sprintf(error_line,"label name:%s",(*l_ent).leble_name);
		insertPrint(print_list,error_line,error);
		insertPrint(print_list,"----------------------------------------------",error);
	}
